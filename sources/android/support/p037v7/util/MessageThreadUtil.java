...
import android.support.p034v4.content.ParallelExecutorCompat;
import android.support.p037v7.util.ThreadUtil;
import android.support.p037v7.util.TileList;
    ...
    public ThreadUtil.MainThreadCallback<T> getMainThreadProxy(final ThreadUtil.MainThreadCallback<T> mainThreadCallback) {
        return new ThreadUtil.MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new Runnable() {
                public void run() {
                    SyncQueueItem next = C05191.this.mQueue.next();
                    while (next != null) {
                        switch (next.what) {
                            case 1:
    ...
    public ThreadUtil.BackgroundCallback<T> getBackgroundProxy(final ThreadUtil.BackgroundCallback<T> backgroundCallback) {
        return new ThreadUtil.BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new Runnable() {
                public void run() {
                    while (true) {
                        SyncQueueItem next = C05212.this.mQueue.next();
                        if (next != null) {
                            switch (next.what) {
                                case 1:
                                    C05212.this.mQueue.removeMessages(1);
                                    backgroundCallback.refresh(next.arg1);
                                    break;
                                case 2:
                                    C05212.this.mQueue.removeMessages(2);
                                    C05212.this.mQueue.removeMessages(3);
                            ...
                            C05212.this.mBackgroundRunning.set(false);
                            return;
                        }
                    }
                }

package com.google.android.gms.internal.ads;

import android.app.Activity;
import android.content.Context;
import android.support.annotation.VisibleForTesting;
import android.support.p034v4.media.session.PlaybackStateCompat;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

public class aww extends awv {

    /* renamed from: s */
    private static final Object f2999s = new Object();

    /* renamed from: t */
    private static final String f3000t = aww.class.getSimpleName();
    @VisibleForTesting

    /* renamed from: u */
    private static boolean f3001u = false;

    /* renamed from: v */
    private static long f3002v = 0;

    /* renamed from: r */
    protected boolean f3003r = false;

    /* renamed from: w */
    private String f3004w;

    /* renamed from: x */
    private boolean f3005x = false;

    /* renamed from: y */
    private boolean f3006y = false;
    @VisibleForTesting

    /* renamed from: z */
    private axz f3007z;

    /* renamed from: a */
    protected static synchronized void m4356a(Context context, boolean z) {
        synchronized (aww.class) {
            if (!f3001u) {
                f3002v = System.currentTimeMillis() / 1000;
                f2979a = m4358b(context, z);
                f3001u = true;
            }
        }
    }

    protected aww(Context context, String str, boolean z) {
        super(context);
        this.f3004w = str;
        this.f3003r = z;
    }

    /* renamed from: b */
    private static axq m4358b(Context context, boolean z) {
        if (f2979a == null) {
            synchronized (f2999s) {
                if (f2979a == null) {
                    axq a = axq.m4398a(context, "qDz6YvDkhwdxUOtNXedEKNdh2XDWXqUECYckxUUtMRo=", "", z);
                    if (a.mo11375b()) {
                        if (((Boolean) bre.m6321e().mo12778a(C1557p.f5983bl)).booleanValue()) {
                            a.mo11373a("phpjyfBMe8u7C5auGMsy22WXoT6iMDb5qqttOP4sXOBlc73QdE1wdYLJ++PsHndY", "86B2wkBiSbEIJu45HO/BQ/RpWXZRq9YpFeIER87ao7I=", new Class[0]);
                        }
                        a.mo11373a("DPtJycwRqj/e0TdTHULzeUhZhWJ1IU3iwhH90sUbn4eYKEdB5HI7UC0PtJgg2RSN", "Ye7G7hL63+8nOBoyCAHdjfK62rvCOKz3+aC1KA/K9CI=", Context.class);
                        a.mo11373a("1IkHdmwdwv9wD8tmJQC+h66q1PQxvS+V9UrDno8I9cxHFnF0E43buTrJO+G+/gQl", "A9jthiAQc+izVBr4mJNKEZTv7NXyHy0tI8Qg667eCzo=", Context.class);
                        a.mo11373a("2a/4JIVPkDFYMOVDvRgiPhzuBvaMjATDrq7QEolObsd/NV02/K8EoZgGgGExyTEN", "zAquriPjpzqGJfvs/9ulqqLxOML+UEqSlyjOz6Tu5VU=", Context.class);
                        a.mo11373a("5Un4Bxdp8gVIANCZ39+qNPpJwSMKWP26YPuxiQ9CnPTd/FlU4AIzDaH9PI9I+CrN", "4Df+wFSysP9SafTWPUlnClwqa+KS42poxSq2xHfYKTY=", Context.class);
                        a.mo11373a("tSH6JN0/dsR0FAYjQVtAYWAamOw3jEQmk6Hr8jJN+yrtPJ4ZwRFL84dAlVlDBCND", "QIWClwyA1htDcymLB9H+9tV5qeIhlSOa2TsbKDGB4CU=", Context.class);
                        a.mo11373a("M1NS9rNHdzevdCO1BkBQDrEtWalzliBN1oXlHuD/PStxAuVrokFwJ8FTE8R3woq0", "4Nk49DU1yHD/oE1MfAOMbRwhDipAhidA7tdJouMJQrI=", Context.class);
                        a.mo11373a("ALF9qjihH/iYAQtvlhVRCPGKYo/wJshkjPnwFlKHSnSRd1pu6U+lD5JnTKlN/8Dh", "hp8obFMJgqHXmaR5pMrBIn7HvqDUI8ERyuIbnl0iT/M=", Context.class);
                        a.mo11373a("DdZDLurYd8cqlTxFH9iUg4PWLqXdMHMkdrTyCVNVmPtO8AJJSQ+/YGXoMqGNOXWI", "c760NRjk4Ctk2HA0ROzb7VHBeyDIvOdXhTBmaaREC+Y=", Context.class);
                        a.mo11373a("LOklmrwaQ0v3CGeP+ZosklY8SbmufmvNG6auvqsL+bGnLcr54FaTv3bt+awlDR0E", "/Fj70v9BUIdYkRnDi6EwnKWPurvJ9lYtBhSoj7eKwt8=", MotionEvent.class, DisplayMetrics.class);
                        a.mo11373a("BDp2p9FkJRWhLjM5/HGlBtfI2hTpjdS5yJQGMqe31LtNjFTgL+0QOyyChfmPPFl0", "got2az0DwXyMWaLpfVutWfEF1l3X50FJhmqzREDcTA8=", MotionEvent.class, DisplayMetrics.class);
                        a.mo11373a("teIp99oid1pENGWnU++9yOPCIwtIDIwejcakl+LfK5qHHKbt0MJt+8BqbhD17b1n", "NREPPB/9iy6pGr9Pw6YHALTcoiK6Qv0Fu8JVtmSXvzQ=", new Class[0]);
                        a.mo11373a("nm3YsDf+7WO+MEOswfwP/mr6SZsOgwQFOzWfB0E01vRE8SnBNpESCxeGx/E3obzH", "ntbFRCAexgxz9p8O68TxIQjW9W7nZKRNbG91vq8F9ro=", new Class[0]);
                        a.mo11373a("OU2zTkMb3xVWxjElhSJNJtcyjBcBIax5KghmsB2DrHu3HRhgp4Bz1cQIIaak8Abf", "0aMkajKGfxJjxUakLLOcj8JypKv0GmCb3d+C/AP1yzc=", new Class[0]);
                        a.mo11373a("55W269KXvkwdzl5sfb0CcKPmBnPIftmJMRWEHFEIHMVBNbB9RYoQD/JeAhpTyftv", "xRFu2EA0XhZqR4z5v1+z5DRPRDh4cLaTVyZWq8w+32M=", new Class[0]);
                        a.mo11373a("diFehzLUdSyhsDAHjo1jRWbuUSlDLM7ewQWDc6EGa5SkktywcOnUZVN2l852D8v/", "gccLIpNanlTav8umvHfA6CkZesdxV9Cr39ehhj7Z8L8=", new Class[0]);
                        a.mo11373a("HW6DUu9hdQUUJG4hyzA4/jDFe17EFpXdJQ2aqCpScU07e6PXExB1P1rW0uW0StIw", "XCJaUXNSa18os9LIeCcjZdYTv1kZvdxKgQEEsV6JTUc=", new Class[0]);
                        a.mo11373a("qsalJ2RwJdF4/xIua4a94Fnnrn+PF/pR/qVYX8QPAaTQDa6NRA9etEnbnhQWblFD", "G68A6YJ+VRJhgHK56Kx1RoGHOyqehPG7tlyyPwtcr7Q=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        a.mo11373a("dUZXsTQGXogdq7xqgYL8i0iimZLTpa9AoXZpRO79MP7o6nVl+DoNjuCg63H0zXDK", "Vz6KZKN/XNSe7DawDVk5XNlgz1HLOOLd+9M1b4lusVA=", StackTraceElement[].class);
                        a.mo11373a("JpT53kCFZ5UGhS3QgNfIxU7Zo+2b3P+3vnYveTkMBHt4zfnotqnDwQTaAD1ryBW9", "SQmHWbTAzPlYuz5I3xZRZNLM/2+IGX009ow9MnjYfrI=", View.class, DisplayMetrics.class);
                        a.mo11373a("9KTvYPBd7MH2ciAlBu9OHvde4n0Fwv5nDeizahStujRhSUn5FAOJjVUu69hMaCIH", "kiOoUcTqb+PvEdOeeDwNvvUZgbOHeCWSZHc9NNTPPv4=", Context.class, Boolean.TYPE);
                        a.mo11373a("TxRdKOZ0pZeBXpIwiLITX81ZqIx50eBVN3DINE4ZBUPdk4novnMzQn3dOLT7/176", "m2+gcfdQWoY+6hOETk2mr0SsbjGMFJBzTmSItOYkEfM=", View.class, Activity.class, Boolean.TYPE);
                        a.mo11373a("9MXEPmGC/7986z/wLWv2b+1iRSmCMoPNrO7Sy/hYl1riiNMJKXOd189OeOHpltpa", "DjhY2o1Svq1HpINv7cU+bAqV+OA81bMZ6vkVBk0u3II=", Long.TYPE);
                    }
                    f2979a = a;
                }
            }
        }
        return f2979a;
    }

    /* renamed from: a */
    private static axw m4355a(axq axq, MotionEvent motionEvent, DisplayMetrics displayMetrics) {
        Method a = axq.mo11371a("LOklmrwaQ0v3CGeP+ZosklY8SbmufmvNG6auvqsL+bGnLcr54FaTv3bt+awlDR0E", "/Fj70v9BUIdYkRnDi6EwnKWPurvJ9lYtBhSoj7eKwt8=");
        if (a == null || motionEvent == null) {
            throw new axn();
        }
        try {
            return new axw((String) a.invoke((Object) null, new Object[]{motionEvent, displayMetrics}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new axn(e);
        }
    }

    /* access modifiers changed from: protected */
    /* renamed from: a */
    public final akd mo11347a(Context context, ahf ahf) {
        akd akd = new akd();
        if (!TextUtils.isEmpty(this.f3004w)) {
            akd.f1927b = this.f3004w;
        }
        axq b = m4358b(context, this.f3003r);
        if (b.mo11376c() != null) {
            m4357a(mo11349a(b, context, akd, (ahf) null));
        }
        return akd;
    }

    /* access modifiers changed from: protected */
    /* renamed from: a */
    public final akd mo11346a(Context context, View view, Activity activity) {
        List list;
        Long l;
        akd akd = new akd();
        if (!TextUtils.isEmpty(this.f3004w)) {
            akd.f1927b = this.f3004w;
        }
        axq b = m4358b(context, this.f3003r);
        if (!b.mo11375b()) {
            akd.f1936k = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
            list = Arrays.asList(new Callable[]{new ayl(b, akd)});
        } else {
            try {
                axw a = m4355a(b, this.f2980b, this.f2995q);
                akd.f1932g = a.f3072a;
                akd.f1933h = a.f3073b;
                akd.f1934i = a.f3074c;
                if (this.f2994p) {
                    akd.f1946u = a.f3075d;
                    akd.f1947v = a.f3076e;
                }
            } catch (axn unused) {
            }
            ale ale = new ale();
            if (this.f2982d > 0 && axx.m4435a(this.f2995q)) {
                ale.f1987m = Long.valueOf(axx.m4430a(this.f2989k, this.f2995q));
                ale.f1988n = Long.valueOf(axx.m4430a((double) (this.f2992n - this.f2990l), this.f2995q));
                ale.f1989o = Long.valueOf(axx.m4430a((double) (this.f2993o - this.f2991m), this.f2995q));
                ale.f1992r = Long.valueOf(axx.m4430a((double) this.f2990l, this.f2995q));
                ale.f1993s = Long.valueOf(axx.m4430a((double) this.f2991m, this.f2995q));
                if (this.f2994p && this.f2980b != null) {
                    long a2 = axx.m4430a((double) (((this.f2990l - this.f2992n) + this.f2980b.getRawX()) - this.f2980b.getX()), this.f2995q);
                    if (a2 != 0) {
                        ale.f1990p = Long.valueOf(a2);
                    }
                    long a3 = axx.m4430a((double) (((this.f2991m - this.f2993o) + this.f2980b.getRawY()) - this.f2980b.getY()), this.f2995q);
                    if (a3 != 0) {
                        ale.f1991q = Long.valueOf(a3);
                    }
                }
            }
            try {
                axw a4 = mo11348a(this.f2980b);
                ale.f1975a = a4.f3072a;
                ale.f1976b = a4.f3073b;
                ale.f1982h = a4.f3074c;
                if (this.f2994p) {
                    ale.f1977c = a4.f3076e;
                    ale.f1979e = a4.f3075d;
                    ale.f1981g = Integer.valueOf(a4.f3077f.longValue() != 0 ? 1 : 0);
                    if (this.f2983e > 0) {
                        if (axx.m4435a(this.f2995q)) {
                            double d = (double) this.f2988j;
                            double d2 = (double) this.f2983e;
                            Double.isNaN(d);
                            Double.isNaN(d2);
                            l = Long.valueOf(Math.round(d / d2));
                        } else {
                            l = null;
                        }
                        ale.f1978d = l;
                        double d3 = (double) this.f2987i;
                        double d4 = (double) this.f2983e;
                        Double.isNaN(d3);
                        Double.isNaN(d4);
                        ale.f1980f = Long.valueOf(Math.round(d3 / d4));
                    }
                    ale.f1984j = a4.f3080i;
                    ale.f1983i = a4.f3081j;
                    ale.f1985k = Integer.valueOf(a4.f3082k.longValue() != 0 ? 1 : 0);
                }
            } catch (axn unused2) {
            }
            if (this.f2986h > 0) {
                ale.f1986l = Long.valueOf(this.f2986h);
            }
            akd.f1891K = ale;
            if (this.f2982d > 0) {
                akd.f1950y = Long.valueOf(this.f2982d);
            }
            if (this.f2983e > 0) {
                akd.f1949x = Long.valueOf(this.f2983e);
            }
            if (this.f2984f > 0) {
                akd.f1948w = Long.valueOf(this.f2984f);
            }
            if (this.f2985g > 0) {
                akd.f1951z = Long.valueOf(this.f2985g);
            }
            try {
                int size = this.f2981c.size() - 1;
                if (size > 0) {
                    akd.f1892L = new ale[size];
                    for (int i = 0; i < size; i++) {
                        axw a5 = m4355a(f2979a, (MotionEvent) this.f2981c.get(i), this.f2995q);
                        ale ale2 = new ale();
                        ale2.f1975a = a5.f3072a;
                        ale2.f1976b = a5.f3073b;
                        akd.f1892L[i] = ale2;
                    }
                }
            } catch (axn unused3) {
                akd.f1892L = null;
            }
            list = new ArrayList();
            if (b.mo11376c() != null) {
                int n = b.mo11387n();
                list.add(new ayl(b, akd));
                axq axq = b;
                akd akd2 = akd;
                list.add(new ayo(axq, "OU2zTkMb3xVWxjElhSJNJtcyjBcBIax5KghmsB2DrHu3HRhgp4Bz1cQIIaak8Abf", "0aMkajKGfxJjxUakLLOcj8JypKv0GmCb3d+C/AP1yzc=", akd2, n, 1));
                list.add(new ayj(axq, "teIp99oid1pENGWnU++9yOPCIwtIDIwejcakl+LfK5qHHKbt0MJt+8BqbhD17b1n", "NREPPB/9iy6pGr9Pw6YHALTcoiK6Qv0Fu8JVtmSXvzQ=", akd2, f3002v, n, 25));
                int i2 = n;
                list.add(new ayi(axq, "55W269KXvkwdzl5sfb0CcKPmBnPIftmJMRWEHFEIHMVBNbB9RYoQD/JeAhpTyftv", "xRFu2EA0XhZqR4z5v1+z5DRPRDh4cLaTVyZWq8w+32M=", akd2, i2, 44));
                list.add(new ayn(axq, "tSH6JN0/dsR0FAYjQVtAYWAamOw3jEQmk6Hr8jJN+yrtPJ4ZwRFL84dAlVlDBCND", "QIWClwyA1htDcymLB9H+9tV5qeIhlSOa2TsbKDGB4CU=", akd2, i2, 12));
                list.add(new ayp(axq, "M1NS9rNHdzevdCO1BkBQDrEtWalzliBN1oXlHuD/PStxAuVrokFwJ8FTE8R3woq0", "4Nk49DU1yHD/oE1MfAOMbRwhDipAhidA7tdJouMJQrI=", akd2, i2, 3));
                list.add(new aym(axq, "diFehzLUdSyhsDAHjo1jRWbuUSlDLM7ewQWDc6EGa5SkktywcOnUZVN2l852D8v/", "gccLIpNanlTav8umvHfA6CkZesdxV9Cr39ehhj7Z8L8=", akd2, i2, 22));
                list.add(new ayh(axq, "5Un4Bxdp8gVIANCZ39+qNPpJwSMKWP26YPuxiQ9CnPTd/FlU4AIzDaH9PI9I+CrN", "4Df+wFSysP9SafTWPUlnClwqa+KS42poxSq2xHfYKTY=", akd2, i2, 5));
                list.add(new ayw(axq, "ALF9qjihH/iYAQtvlhVRCPGKYo/wJshkjPnwFlKHSnSRd1pu6U+lD5JnTKlN/8Dh", "hp8obFMJgqHXmaR5pMrBIn7HvqDUI8ERyuIbnl0iT/M=", akd2, i2, 48));
                list.add(new ayd(axq, "DdZDLurYd8cqlTxFH9iUg4PWLqXdMHMkdrTyCVNVmPtO8AJJSQ+/YGXoMqGNOXWI", "c760NRjk4Ctk2HA0ROzb7VHBeyDIvOdXhTBmaaREC+Y=", akd2, i2, 49));
                list.add(new ayt(axq, "HW6DUu9hdQUUJG4hyzA4/jDFe17EFpXdJQ2aqCpScU07e6PXExB1P1rW0uW0StIw", "XCJaUXNSa18os9LIeCcjZdYTv1kZvdxKgQEEsV6JTUc=", akd2, i2, 51));
                list.add(new ays(axq, "dUZXsTQGXogdq7xqgYL8i0iimZLTpa9AoXZpRO79MP7o6nVl+DoNjuCg63H0zXDK", "Vz6KZKN/XNSe7DawDVk5XNlgz1HLOOLd+9M1b4lusVA=", akd2, i2, 45, new Throwable().getStackTrace()));
                list.add(new ayx(axq, "JpT53kCFZ5UGhS3QgNfIxU7Zo+2b3P+3vnYveTkMBHt4zfnotqnDwQTaAD1ryBW9", "SQmHWbTAzPlYuz5I3xZRZNLM/2+IGX009ow9MnjYfrI=", akd2, i2, 57, view));
                list.add(new ayr(axq, "9KTvYPBd7MH2ciAlBu9OHvde4n0Fwv5nDeizahStujRhSUn5FAOJjVUu69hMaCIH", "kiOoUcTqb+PvEdOeeDwNvvUZgbOHeCWSZHc9NNTPPv4=", akd2, i2, 61));
                if (((Boolean) bre.m6321e().mo12778a(C1557p.f5977bf)).booleanValue()) {
                    list.add(new ayc(b, "TxRdKOZ0pZeBXpIwiLITX81ZqIx50eBVN3DINE4ZBUPdk4novnMzQn3dOLT7/176", "m2+gcfdQWoY+6hOETk2mr0SsbjGMFJBzTmSItOYkEfM=", akd, n, 62, view, activity));
                }
                if (((Boolean) bre.m6321e().mo12778a(C1557p.f5979bh)).booleanValue()) {
                    list.add(new ayu(b, "9MXEPmGC/7986z/wLWv2b+1iRSmCMoPNrO7Sy/hYl1riiNMJKXOd189OeOHpltpa", "DjhY2o1Svq1HpINv7cU+bAqV+OA81bMZ6vkVBk0u3II=", akd, n, 53, this.f3007z));
                }
            }
        }
        m4357a((List<Callable<Void>>) list);
        return akd;
    }

    /* access modifiers changed from: protected */
    /* renamed from: a */
    public List<Callable<Void>> mo11349a(axq axq, Context context, akd akd, ahf ahf) {
        int n = axq.mo11387n();
        ArrayList arrayList = new ArrayList();
        if (!axq.mo11375b()) {
            akd.f1936k = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
            return arrayList;
        }
        akd akd2 = akd;
        axq axq2 = axq;
        akd akd3 = akd;
        arrayList.add(new ayg(axq2, "qsalJ2RwJdF4/xIua4a94Fnnrn+PF/pR/qVYX8QPAaTQDa6NRA9etEnbnhQWblFD", "G68A6YJ+VRJhgHK56Kx1RoGHOyqehPG7tlyyPwtcr7Q=", akd3, n, 27, context, ahf));
        arrayList.add(new ayj(axq2, "teIp99oid1pENGWnU++9yOPCIwtIDIwejcakl+LfK5qHHKbt0MJt+8BqbhD17b1n", "NREPPB/9iy6pGr9Pw6YHALTcoiK6Qv0Fu8JVtmSXvzQ=", akd3, f3002v, n, 25));
        int i = n;
        arrayList.add(new ayo(axq2, "OU2zTkMb3xVWxjElhSJNJtcyjBcBIax5KghmsB2DrHu3HRhgp4Bz1cQIIaak8Abf", "0aMkajKGfxJjxUakLLOcj8JypKv0GmCb3d+C/AP1yzc=", akd3, i, 1));
        arrayList.add(new ayq(axq2, "2a/4JIVPkDFYMOVDvRgiPhzuBvaMjATDrq7QEolObsd/NV02/K8EoZgGgGExyTEN", "zAquriPjpzqGJfvs/9ulqqLxOML+UEqSlyjOz6Tu5VU=", akd3, i, 31));
        arrayList.add(new ayv(axq2, "nm3YsDf+7WO+MEOswfwP/mr6SZsOgwQFOzWfB0E01vRE8SnBNpESCxeGx/E3obzH", "ntbFRCAexgxz9p8O68TxIQjW9W7nZKRNbG91vq8F9ro=", akd3, i, 33));
        arrayList.add(new aye(axq2, "1IkHdmwdwv9wD8tmJQC+h66q1PQxvS+V9UrDno8I9cxHFnF0E43buTrJO+G+/gQl", "A9jthiAQc+izVBr4mJNKEZTv7NXyHy0tI8Qg667eCzo=", akd3, i, 29, context));
        arrayList.add(new ayh(axq2, "5Un4Bxdp8gVIANCZ39+qNPpJwSMKWP26YPuxiQ9CnPTd/FlU4AIzDaH9PI9I+CrN", "4Df+wFSysP9SafTWPUlnClwqa+KS42poxSq2xHfYKTY=", akd3, i, 5));
        arrayList.add(new ayn(axq2, "tSH6JN0/dsR0FAYjQVtAYWAamOw3jEQmk6Hr8jJN+yrtPJ4ZwRFL84dAlVlDBCND", "QIWClwyA1htDcymLB9H+9tV5qeIhlSOa2TsbKDGB4CU=", akd3, i, 12));
        arrayList.add(new ayp(axq2, "M1NS9rNHdzevdCO1BkBQDrEtWalzliBN1oXlHuD/PStxAuVrokFwJ8FTE8R3woq0", "4Nk49DU1yHD/oE1MfAOMbRwhDipAhidA7tdJouMJQrI=", akd3, i, 3));
        arrayList.add(new ayi(axq2, "55W269KXvkwdzl5sfb0CcKPmBnPIftmJMRWEHFEIHMVBNbB9RYoQD/JeAhpTyftv", "xRFu2EA0XhZqR4z5v1+z5DRPRDh4cLaTVyZWq8w+32M=", akd3, i, 44));
        arrayList.add(new aym(axq2, "diFehzLUdSyhsDAHjo1jRWbuUSlDLM7ewQWDc6EGa5SkktywcOnUZVN2l852D8v/", "gccLIpNanlTav8umvHfA6CkZesdxV9Cr39ehhj7Z8L8=", akd3, i, 22));
        arrayList.add(new ayw(axq2, "ALF9qjihH/iYAQtvlhVRCPGKYo/wJshkjPnwFlKHSnSRd1pu6U+lD5JnTKlN/8Dh", "hp8obFMJgqHXmaR5pMrBIn7HvqDUI8ERyuIbnl0iT/M=", akd3, i, 48));
        arrayList.add(new ayd(axq2, "DdZDLurYd8cqlTxFH9iUg4PWLqXdMHMkdrTyCVNVmPtO8AJJSQ+/YGXoMqGNOXWI", "c760NRjk4Ctk2HA0ROzb7VHBeyDIvOdXhTBmaaREC+Y=", akd3, i, 49));
        arrayList.add(new ayt(axq2, "HW6DUu9hdQUUJG4hyzA4/jDFe17EFpXdJQ2aqCpScU07e6PXExB1P1rW0uW0StIw", "XCJaUXNSa18os9LIeCcjZdYTv1kZvdxKgQEEsV6JTUc=", akd3, i, 51));
        arrayList.add(new ayr(axq2, "9KTvYPBd7MH2ciAlBu9OHvde4n0Fwv5nDeizahStujRhSUn5FAOJjVUu69hMaCIH", "kiOoUcTqb+PvEdOeeDwNvvUZgbOHeCWSZHc9NNTPPv4=", akd3, i, 61));
        return arrayList;
    }

    /* renamed from: a */
    private static void m4357a(List<Callable<Void>> list) {
        ExecutorService c;
        if (f2979a != null && (c = f2979a.mo11376c()) != null && !list.isEmpty()) {
            try {
                c.invokeAll(list, ((Long) bre.m6321e().mo12778a(C1557p.f5976be)).longValue(), TimeUnit.MILLISECONDS);
            } catch (InterruptedException e) {
                Log.d(f3000t, String.format("class methods got exception: %s", new Object[]{axx.m4433a((Throwable) e)}));
            }
        }
    }

    /* access modifiers changed from: protected */
    /* renamed from: a */
    public final axw mo11348a(MotionEvent motionEvent) {
        Method a = f2979a.mo11371a("BDp2p9FkJRWhLjM5/HGlBtfI2hTpjdS5yJQGMqe31LtNjFTgL+0QOyyChfmPPFl0", "got2az0DwXyMWaLpfVutWfEF1l3X50FJhmqzREDcTA8=");
        if (a == null || motionEvent == null) {
            throw new axn();
        }
        try {
            return new axw((String) a.invoke((Object) null, new Object[]{motionEvent, this.f2995q}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new axn(e);
        }
    }

    /* access modifiers changed from: protected */
    /* renamed from: a */
    public final long mo11345a(StackTraceElement[] stackTraceElementArr) {
        Method a = f2979a.mo11371a("dUZXsTQGXogdq7xqgYL8i0iimZLTpa9AoXZpRO79MP7o6nVl+DoNjuCg63H0zXDK", "Vz6KZKN/XNSe7DawDVk5XNlgz1HLOOLd+9M1b4lusVA=");
        if (a == null || stackTraceElementArr == null) {
            throw new axn();
        }
        try {
            return new axo((String) a.invoke((Object) null, new Object[]{stackTraceElementArr})).f3040a.longValue();
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new axn(e);
        }
    }

    public final void zzb(View view) {
        if (((Boolean) bre.m6321e().mo12778a(C1557p.f5979bh)).booleanValue()) {
            if (this.f3007z == null) {
                this.f3007z = new axz(f2979a, view);
            } else {
                this.f3007z.mo11394a(view);
            }
        }
    }
}
